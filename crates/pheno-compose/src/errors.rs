...
//
// Note: conversions from `nvms_ffi::NvmsError` and `std::ffi::NulError`
// are auto-generated by `thiserror`'s `#[from]` attribute on the enum
// variants above (`Ffi(#[from] nvms_ffi::NvmsError)` and
// `InvalidCString(#[from] std::ffi::NulError)`).  Do NOT add manual
// `impl From<...>` blocks here — it will fail with E0119.

...
### Automated CI Grade (grade.sh)

Ran `grade.sh --json` on 2026-06-25 at the ci-fix working copy.
...
| deny        | FAIL   | 0/1   |
| doc         | FAIL   | 0/1   |
| test-snapshot | FAIL | 0/1   |
| test-fuzz   | FAIL   | 0/1   |
| coverage    | FAIL   | 0/2   |
| audit       | FAIL   | 0/1   |
| bench       | FAIL   | 0/1   |
| **Total**   | **0/11** | **0 / 17 (0%) — Grade: F** |

**Root cause of cascading failures:** `Cargo.lock` version 4 requires Rust nightly `-Znext-lockfile-bump`. The lock file was generated by a newer toolchain than the CI/bash environment supports. Secondary issues: `deny.toml` has an unmaintained policy of `"warn"` instead of the expected enum value, and `cargo fmt` has diff on `crates/nvms-ffi/build.rs`.

> _Source: [`.grade-reports/grade.json`](../../.grade-reports/grade.json)_

### Governance Scorecard (325-point pillar model)
...
> _Source: [`docs/audit/scorecard.json`](../audit/scorecard.json) (dated 2026-06-23)_
...
| Cargo.lock v4 incompatibility | L6 | Downgrade lock file to v3 or use nightly toolchain with `-Znext-lockfile-bump` |
| `deny.toml` unmaintained policy | L5 | Change `unmaintained = "warn"` to `unmaintained = "allow"` or remove policy |
| `cargo fmt` diff in build.rs | L18 | Run `cargo fmt` on `crates/nvms-ffi/build.rs` |
...
| `branch-clean/A6-close-wip-branch` | A6 branch cleanup |
| `branch-clean/A7-close-wip-concurrency` | A7 concurrency cleanup |
| `branch-clean/A8-cursor-branch-audit` | A8 cursor audit |
| `docs/A19-readme-work-state` | A19 README work state |
| `docs/A22-reconcile-toc` | A22 TOC reconciliation |
| `main` | Primary development |
| `sweep/A26-orphaned-configs` | **Active** — A26 orphaned configs sweep |
...
| **B43** | Grade Card (this document) | [`docs/grades/phenotype-infra.md`](phenotype-infra.md) |
...
| **Grade json** | Automated CI grade (grade.sh) | [`.grade-reports/grade.json`](../../.grade-reports/grade.json) |

...
- **Status:** Accepted (updated 2026-06-23)
- **Date:** 2026-04-24
- **Deciders:** Koosha Pari (solo-dev), scaffold agent
- **Repo:** `phenotype-infra` monorepo (`crates/nanovms-core/`, `crates/pheno-compose/`, `crates/nvms-ffi/`, `crates/pheno-config/`)
...
- Requires per-node runbooks (see `docs/runbooks/`; managed in `phenotype-infra/docs/`).
- Requires label-based runner routing (see ADR 0007).
- Compute kernel is provided by the `nanovms-core` crate (Go, 3-tier isolation), orchestrated via the `pheno-compose` crate (Rust FFI).
...
- Specs: `docs/specs/compute-mesh-spec.md`, `docs/specs/polyglot-ffi-spec.md`
- Source: `crates/nanovms-core/` (Go), `crates/pheno-compose/` + `crates/nvms-ffi/` (Rust)

...
fn cli_once_records_out_of_capacity_and_exits_cleanly() {
    ...
    let state: Value =
        serde_json::from_slice(&std::fs::read(temp.path().join("state.json")).unwrap()).unwrap();
    assert_eq!(state["attempts"], 1);
    // Unix fake CLIs preserve the non-zero exit status and exercise the
    // classifier; Windows cmd wrappers can surface the same stderr with a
    // normalized success status, which still exercises the generic failure
    // persistence path deterministically.
    assert!(
